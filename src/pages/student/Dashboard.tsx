  ...
  return (
    <div className="space-y-10">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-[#94a3b8] text-lg font-medium">Bienvenue sur votre espace d'apprentissage.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <motion.div
            ...
            className="bg-[#1e293b] border border-[#334155] rounded-[2rem] p-6 hover:border-[#334155]/50 transition-all"
          >
            <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mb-4", stat.bgColor, stat.borderColor, "border")}>
              <stat.icon size={24} className={cn("text-white")} />
            </div>
            <p className="text-[10px] uppercase font-bold text-[#64748b] tracking-widest mb-2">{stat.label}</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-black text-white tracking-tight uppercase">Quiz Disponibles</h2>
          <button className="text-[10px] font-black text-[#2563eb] uppercase tracking-widest hover:underline px-4 py-2 bg-[#2563eb]/10 rounded-full">
            Voir tout
          </button>
        </div>
            ...
            {Array.isArray(quizzes['results']) && quizzes['results'].slice(0, 6).map((quiz, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    "group relative bg-[#1e293b] border rounded-[2rem] p-6 flex flex-col transition-all duration-300",
                    expired ? "opacity-60 border-[#334155]" : "border-[#334155] hover:border-[#2563eb]/50 hover:shadow-2xl hover:shadow-[#2563eb]/10"
                  )}
                >
                  <div className="flex flex-wrap gap-2 mb-4">
                    <div className="bg-[#2563eb]/10 text-[#2563eb] px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-[#2563eb]/20">
                      {quiz.category}
                    </div>
                    <div className="bg-purple-500/10 text-purple-500 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-purple-500/20">
                      {quiz.difficulty}
                    </div>
                    {expired && (
                      <div className="bg-red-500/10 text-red-500 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-red-500/20 flex items-center gap-1">
                        <AlertCircle size={8} /> Expiré
                      </div>
                    )}
                  </div>

                  <h3 className="text-lg font-black mb-2 group-hover:text-[#2563eb] transition-colors">{quiz.title}</h3>
                  <p className="text-[#94a3b8] text-xs leading-relaxed mb-4 line-clamp-2">{quiz.description}</p>

                  <div className="grid grid-cols-2 gap-3 mb-4 flex-1">
                    <div className="bg-[#0f172a] rounded-xl p-3 border border-[#334155]/50">
                      <p className="text-[9px] uppercase font-bold text-[#64748b] tracking-widest mb-1">Questions</p>
                    ...
                    <div className="bg-[#0f172a] rounded-xl p-3 border border-[#334155]/50">
                      <p className="text-[9px] uppercase font-bold text-[#64748b] tracking-widest mb-1">Essais</p>
                      <div className="flex items-center gap-2 font-bold text-[#f8fafc] text-sm">
                  ...
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-center gap-2 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all transform active:scale-95",
                      canStart 
